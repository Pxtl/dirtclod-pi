...
if [ -d /root/ssh-mitm-ssh ]; then
    echo "Initializing /root/.ssh from /root/ssh-mitm-ssh..."
    cp -r /root/ssh-mitm-ssh/* /root/.ssh/
fi

### 3. Initialize git user’s SSH directory from seed volume
if [ -d /root/ssh-mitm-ssh ]; then
    echo "Initializing /home/git/.ssh from /root/ssh-mitm-ssh..."
    mkdir -p /home/git/.ssh
    cp -r /root/ssh-mitm-ssh/* /home/git/.ssh/
fi
